...
SCRIPT_LIST=(heysolo.sh install.sh mt5.sh uninstall.sh win/wine-compliance.sh win/test-compliance.sh win/install-compliance.sh)
if [[ -t 1 ]]; then
...
bash "${SCRIPTS_DIR}/win/wine-compliance.sh"
else
...
say "Upload into ${BOLD}${MQL5_LOCAL_DIR:-/opt/heysolo/bot/MT5}${NC}/{Experts,Include,Indicators,set,Templates}"
say "Upload HeySoloATM_Sets files into ${BOLD}${HEYSOLO_SETS_LOCAL_DIR:-/opt/heysolo/bot/MT5/HeySoloATM_Sets}${NC}"
